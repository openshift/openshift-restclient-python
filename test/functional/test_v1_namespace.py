...
@pytest.fixture()
def k8s_namespace(admin_ansible_helper, create_params):
...
def test_create_namespace(admin_ansible_helper, create_params, k8s_namespace, obj_compare):
    obj_compare(admin_ansible_helper, k8s_namespace, create_params)


def test_get_namespace(admin_ansible_helper, k8s_namespace):
    name = k8s_namespace.metadata.name
    k8s_obj = admin_ansible_helper.get_object(name, None)
    assert k8s_obj is not None


def test_patch_namespace(admin_ansible_helper, k8s_namespace, patch_params, obj_compare):
    ...
    match, _ = admin_ansible_helper.objects_match(existing_obj, updated_obj)
    assert not match
    new_obj = admin_ansible_helper.patch_object(name, None, updated_obj)
    assert new_obj is not None
    obj_compare(admin_ansible_helper, new_obj, patch_params)


def test_replace_namespace(admin_ansible_helper, k8s_namespace, replace_params, obj_compare):
    name = replace_params.get('name')
    request_body = admin_ansible_helper.request_body_from_params(replace_params)
    k8s_obj = admin_ansible_helper.replace_object(name, None, body=request_body)
    obj_compare(admin_ansible_helper, k8s_obj, replace_params)


def test_remove_namespace(admin_ansible_helper, k8s_namespace):
    name = k8s_namespace.metadata.name
    admin_ansible_helper.delete_object(name, None)
    k8s_obj = admin_ansible_helper.get_object(name, None)
