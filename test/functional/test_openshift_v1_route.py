...
@pytest.fixture()
def route(openshift_ansible_helper, create_params):
...
def test_create_route(openshift_ansible_helper, create_params, route, obj_compare):
    obj_compare(openshift_ansible_helper, route, create_params)


def test_get_route(openshift_ansible_helper, route):
...
def test_patch_route(openshift_ansible_helper, route, patch_params, obj_compare):
...
def test_replace_route(openshift_ansible_helper, route, replace_params, obj_compare):
...
def test_remove_route(openshift_ansible_helper, route):
    name = route.metadata.name
    namespace = route.metadata.namespace
    openshift_ansible_helper.delete_object(name, namespace)
    k8s_obj = openshift_ansible_helper.get_object(name, namespace)
