...
@pytest.fixture()
def persistent_volume_claim(ansible_helper, create_params):
...
def test_create_persistent_volume_claim(ansible_helper, create_params, persistent_volume_claim, obj_compare):
    obj_compare(ansible_helper, persistent_volume_claim, create_params)


def test_get_persistent_volume_claim(ansible_helper, persistent_volume_claim):
...
def test_remove_persistent_volume_claim(ansible_helper, persistent_volume_claim):
    name = persistent_volume_claim.metadata.name
    namespace = persistent_volume_claim.metadata.namespace
    ansible_helper.delete_object(name, namespace)
    k8s_obj = ansible_helper.get_object(name, namespace)
