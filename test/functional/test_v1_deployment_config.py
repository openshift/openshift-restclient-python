...
def test_create_deployment(ansible_helper, create_tasks, obj_compare, create_namespace):
...
def test_get_deployment(ansible_helper, create_tasks):
    parameters = create_tasks['create']
    namespace = parameters.get('namespace')
...
def test_patch_deployment(ansible_helper, patch_tasks, obj_compare):
    parameters = patch_tasks['patch']
    namespace = parameters.get('namespace')
    existing_obj = ansible_helper.get_object(parameters['name'], namespace)
    print("Existing:")
    print(json.dumps(existing_obj.to_dict(), indent=4))
    ...
    assert not match
    print("Updated:")
    print(json.dumps(updated_obj.to_dict(), indent=4))
