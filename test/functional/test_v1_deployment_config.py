...
def test_create_deployment_config(ansible_helper, create_params, deployment_config, obj_compare):
    obj_compare(ansible_helper, deployment_config, create_params)


def test_get_deployment(ansible_helper, deployment_config):
...
def test_patch_deployment(ansible_helper, deployment_config, patch_params, obj_compare):
...
def test_remove_deployment(ansible_helper, deployment_config):
    namespace = deployment_config.metadata.namespace
    name = deployment_config.metadata.name
    ansible_helper.delete_object(name, namespace)
    k8s_obj = ansible_helper.get_object(name, namespace)
