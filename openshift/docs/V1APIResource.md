...
**singular_name** | **str** | singularName is the singular name of the resource.  This allows openshift.clients to handle plural and singular opaquely. The singularName is more correct for reporting status on a single item and both singular and plural are allowed from the kubectl CLI interface. | 
